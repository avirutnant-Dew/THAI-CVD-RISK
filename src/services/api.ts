...
export async function fetchPackages(): Promise<HealthPackage[]> {
  ...
  return rows.map((row) => ({ packageId: String(row.package_id), packageName: String(row.package_name), shortDescription: String(row.short_description), fullDescription: String(row.full_description), normalPrice: Number(row.normal_price) || undefined, promoPrice: Number(row.promo_price) || undefined, imageUrl: String(row.package_id) === 'CV004' ? '/packages/cv004-advanced.png' : String(row.image_url || ''), detailUrl: String(row.detail_url || ''), includedTests: Array.isArray(row.included_tests) ? row.included_tests.map(String) : [], packageTags: Array.isArray(row.package_tags) ? row.package_tags.map(String) : [], minAge: Number(row.min_age), maxAge: Number(row.max_age), active: true, priority: Number(row.priority), demoOnly: false }))
}
